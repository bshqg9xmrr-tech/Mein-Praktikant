...
**stand nach der klärungsrunde "app-flow neu gedacht"**: das komplette, mit dem nutzer abgestimmte neu-konzept aus `architecture.md` §2.2 ist jetzt umgesetzt — login-pflicht + onboarding, die drei bereiche strom/kompass/verlauf, sowie goal-modell/gamification-anpassungen im datenmodell. die einzelnen umsetzungsschritte stehen ausführlich in `CHANGELOG.md` ("login-pflicht + onboarding-wizard", "neue navigation + 'strom'", "'kompass' gefüllt", "'verlauf' gefüllt"). weiterhin bewusst nicht enthalten bleibt die eigentliche ki-schicht (`architecture.md` §2.1) — siehe "was bewusst noch nicht echt ist" unten.
...
3. **verlauf** (`js/verlauf.js`) — ersetzt übersicht + tagebuch-liste + habit-historie: ein vertikaler zeitstrahl aus erledigten todos, abgehakten habits und tagebuch-einträgen (siehe unten).
...
- **verlauf** (`js/verlauf.js`, neu): ein vertikaler zeitstrahl der letzten 21 tage (praktikabler zeitraum, nicht zu lang) — pro tag mit irgendeiner aktivität (erledigte todos, abgehakte habits, ein tagebuch-eintrag) ein kompakter eintrag: anzahl erledigter todos, welche habits abgehakt wurden, ein gekürzter tagebuch-auszug. tage ganz ohne aktivität werden übersprungen; der heutige tag bleibt immer sichtbar (fester einstiegspunkt für "heute reflektieren"). **filter-pills** (alle/ziele/habits/stimmung) blenden rein clientseitig nicht relevante teile jedes tages-eintrags aus. der **mood-punkt** auf der zeitstrahl-linie ist zugleich der "punkt pro tag" aus dem timeline-design — seine farbe kommt aus einer einfachen, dokumentierten lokalen heuristik (siehe unten), **keine echte stimmungserkennung**. eine **"aufgefallen"-karte** (im `.ai-banner`-stil) zeigt höchstens eine einzelne, lokal berechnete auffälligkeit (z. b. ein habit, das länger nicht abgehakt wurde als sonst üblich, oder ein deutlicher wochenvergleich bei erledigten todos) — erscheint nur, wenn die datenlage dafür ausreicht, sonst gar nicht (lieber nichts zeigen als etwas erfinden).
- **"heute reflektieren" / "habits heute abhaken"** (`verlauf.js`, neu): öffnen die **unveränderte** render-logik aus `journal.js` bzw. `habits.js` in einem modal (`ui.js#openModal`, das jetzt einen optionalen `onClose`-callback unterstützt, damit `verlauf.js` nach dem schließen neu rendert) — echte wiederverwendung statt neubau, inkl. der bereits vorhandenen echten streak-berechnung und der strukturierten tagebuch-felder mit automatisch übernommenen erledigten todos.
- **tagebuch** (`journal.js`) und **habits** (`habits.js`): unverändert, jetzt aber wieder erreichbar — über die beiden obigen einstiegspunkte in `verlauf.js` statt eines eigenen tabs.
- **übersicht**: alle zahlen sind echte, aus den lokalen daten berechnete werte — vorhanden in `overview.js`, aktuell nicht per tab/modal erreichbar (die relevanten kennzahlen sind inhaltlich in den neuen `verlauf.js`-zeitstrahl übergegangen).
...
- **der mood-punkt im "verlauf"-zeitstrahl ist keine echte stimmungserkennung** — es gibt (noch) kein eigenes stimmungs-feld im datenmodell. existiert ein tagebuch-eintrag für den tag, wird die punktfarbe aus dessen bereits vom nutzer selbst gewählten `feelings` (fest zugeordnet zu "eher positiv"/"eher angespannt") bzw. ersatzweise aus einer einfachen wortlisten-suche im freitext abgeleitet — dieselbe ehrlichkeits-konvention wie `strom.js#classify()`. ohne eindeutigen ausschlag (oder ganz ohne tagebuch-eintrag, aber mit anderer aktivität an dem tag) ist der punkt neutral-grau ("gemischt/unklar" bzw. "keine angabe") statt eine stimmung zu erfinden.
- **die "aufgefallen"-karte in "verlauf" ist eine einfache, rein lokal berechnete statistik, keine ki** — entweder ein habit, das deutlich länger nicht abgehakt wurde als sein bisheriger durchschnittlicher rhythmus, oder ein klarer wochenvergleich bei erledigten todos. reicht die datenlage nicht (z. b. frisch installierte app), erscheint gar keine karte — im UI als "lokale statistik, keine ki" beschriftet.
