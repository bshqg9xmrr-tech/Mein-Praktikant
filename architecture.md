...
1. **offline-first**: alle kernfunktionen (todos, ziele, notizen, tagebuch, habits) funktionieren vollständig ohne internet. cloud-sync ist ein zusatz, kein voraussetzung. **login ist der empfohlene erste schritt** (passwortlos, siehe §4.8), nicht länger ein hartes muss — er bleibt der geplante normalfall (u. a. weil challenges, §4.13, echte konten brauchen), aber die **infrastruktur dafür** (ein eingerichtetes cloud-projekt) darf nie zur voraussetzung werden, um die app überhaupt zu öffnen. **korrektur nach einer review-runde** (siehe `app/CHANGELOG.md` "review-korrekturen"): eine erste umsetzung dieses gates hatte genau das getan — kein ausweg ohne vollständiges supabase-setup, ein sackgassen-zustand für einen ersten test. behoben über einen expliziten "erstmal lokal ausprobieren"-opt-out auf dem setup-bildschirm (`app/js/auth-gate.js`); login/cloud lassen sich danach jederzeit über die einstellungen nachholen.
...
**login ist der geplante normalfall, einmalig pro gerät** (siehe §4.8): passwortlos (magic-link — **ausdrücklich kein passwort**, adhs-freundlichkeit hat vorrang vor der ursprünglichen idee eines klassischen passwort-logins), danach normaler offline-first-betrieb. **korrektur nach einer review-runde** (neu, siehe §1/§4.8/`app/CHANGELOG.md` "review-korrekturen"): er ist dabei nicht mehr hart erzwungen — ein "erstmal lokal ausprobieren"-opt-out auf dem setup-bildschirm verhindert, dass ein fehlendes cloud-projekt zur sackgasse für einen ersten test wird.
...
### 4.8 auth & billing
- **login ist der geplante normalfall, einmalig pro gerät** (§2.2) — der erste bildschirm der app führt standardmäßig zum login. **ausdrücklich passwortlos** (magic-link): markus hatte zunächst klassisches "mail + passwort" vorgeschlagen, sich nach rückfrage aber bewusst für passwortlos entschieden (kein passwort zum merken/tippen — adhs-typischer reibungspunkt). grund für den login als normalfall: die app soll wissen, wessen daten das sind (für challenges, §4.13, und für den cloud-sync), nicht länger eine reine "cloud ist optionales extra"-haltung wie in der ursprünglichen v1-planung. nach dem login läuft die app wieder normal offline-first weiter (leitprinzip §1).
- **korrektur nach einer review-runde** (neu, siehe §1, `app/CHANGELOG.md` "review-korrekturen"): "login als normalfall" heißt ausdrücklich **nicht** "cloud-infrastruktur-setup als voraussetzung". eine erste umsetzung des web-prototyp-gates (`app/js/auth-gate.js`) hatte den setup-bildschirm ohne ausweg blockierend gestaltet — wer noch kein supabase-projekt eingerichtet hatte, kam gar nicht erst in die app. das widersprach leitprinzip §1 und wurde korrigiert: der setup-bildschirm bietet jetzt zusätzlich "erstmal lokal ausprobieren" (setzt nur ein lokales flag, keine cloud nötig) als gleichwertigen, klar sichtbaren zweiten weg. login/cloud bleiben der **empfohlene** weg (v. a. für §4.13/geräteübergreifenden sync) und lassen sich jederzeit später über die einstellungen nachholen — sie sind aber kein hartes muss mehr, um die app zu öffnen.
